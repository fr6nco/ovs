...
odp_action_len(uint16_t type)
{
    ...
    case OVS_ACTION_ATTR_INC_SEQ: return sizeof(struct ovs_action_inc_seq);
    case OVS_ACTION_ATTR_INC_ACK: return sizeof(struct ovs_action_inc_ack);
                 ...
                 struct ofpbuf *actions)
{
...

    {
        ...
        if(ovs_scan(s, "inc_seq(%"SCNi32")%n", &increment, &n)) {
    ...
    {
        struct ovs_action_inc_ack ack;
        uint32_t increment = 0;
        int n = -1;

        if(ovs_scan(s, "inc_ack(%"SCNi32")%n", &increment, &n)) {
            ack.increment = increment;
            nl_msg_put_unspec(actions, OVS_ACTION_ATTR_INC_ACK, &ack, sizeof ack);
            return n;
        }
    }
